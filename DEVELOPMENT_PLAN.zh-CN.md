...
- 已新增 `/master-data/products` 商品列表，并接入关键词 / 公司 / 仓库 / 状态 / 分类 / 品牌 / 仅有库存筛选、新增、编辑、启用、停用、批量启停、批量修改分类 / 品牌、当前筛选结果 CSV 导出、图片上传、图片替换和图片删除；商品详情页已支持多条码列表、新增条码、删除条码和设置主条码。商品图片显示已补 `/files/` dev proxy、URL 版本参数和统一无图 / 加载失败占位，避免后端已返回图片但前端不显示、继续读取旧缓存，或图片为空时页面完全隐藏图片能力；AI 当前商品详情支持直接上传、替换和删除后刷新正式数据。
