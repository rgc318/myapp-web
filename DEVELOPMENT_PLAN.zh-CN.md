...
阶段 7 已完成第一批高频写操作：销售 / 采购订单详情可创建下游单据、登记收付款并取消订单；销售发货单、销售发票、采购收货单、采购发票详情可取消单据；销售 / 采购发票详情可取消最近收款 / 付款。后续阶段 7 仍可继续补待处理确认、表单化参数和主数据轻量编辑。
...
- Web 端第一批写操作已接入；复杂表单参数、待处理确认和主数据编辑尚未接入。
