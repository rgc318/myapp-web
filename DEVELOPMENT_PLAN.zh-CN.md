...
- 已新增 `/master-data/customers` 客户治理第一版，并接入关键词 / 状态 / 分组筛选、新增、编辑、启用、停用、详情抽屉、主联系人 / 主地址维护、最近使用地址展示、当前筛选结果 CSV 导出和 CSV 批量导入。
- 已新增 `/master-data/suppliers` 供应商治理第一版，并接入关键词 / 状态 / 分组筛选、新增、编辑、启用、停用、详情抽屉、主联系人 / 主地址维护、最近使用地址展示、当前筛选结果 CSV 导出和 CSV 批量导入。
...
- 当前商品、客户、供应商、仓库和计量单位已开放常用治理能力；商品图片维护已接入后端 media gateway；库存目标数量调整和库存转仓已接入，完整批量盘点单仍归后续库存写操作模块。
- 客户和供应商已升级为企业级第一版；联系人和地址多条独立维护、信用 / 账期 / 付款条款 / 税务 / 交易历史聚合、应收应付钻取、标签归属和审计记录仍归后续主数据治理模块。
...
- Web 端第一批写操作已接入；销售 / 采购下游单据已支持明细行本次数量，采购订单新建、编辑和退货已接入，商品、客户、供应商、仓库和计量单位常用治理已接入，商品图片上传 / 替换 / 删除已接入，库存目标数量调整和库存转仓已接入，销售 / 采购核心单据打印预览和 PDF 下载已接入，待处理确认已接入，完整批量盘点单尚未接入。
