...
- 已新增 `/master-data/warehouses` 仓库列表，并接入关键词 / 公司 / 状态 / 类型筛选、新增、编辑、启用和停用；仓库表单已覆盖会计科目、仓库类型、默认在途仓库、拒收仓标记、客户归属、联系方式和地址等 ERPNext 原生治理字段。
- 当前商品、客户、供应商和计量单位已开放轻量维护；商品图片维护已接入后端 media gateway；库存目标数量调整和库存转仓已接入，完整批量盘点单仍归后续库存写操作模块。
- 客户和供应商当前仍是轻量主数据维护，不是完整 CRM / 供应商治理模块；客户 / 供应商详情、联系人和地址多条维护、信用 / 账期 / 税务 / 交易历史聚合、应收应付钻取、导入导出和审计记录仍归后续主数据治理模块。
- 仓库管理已接入 `/master-data/warehouses` 的 ERPNext 原生基础治理字段；库位 / 容量、负责人、默认成本中心、仓库权限、导入导出、审计记录和更细粒度治理仍归后续模块。
...
- Web 端第一批写操作已接入；销售 / 采购下游单据已支持明细行本次数量，采购订单新建、编辑和退货已接入，商品、客户、供应商和计量单位轻量编辑已接入，商品图片上传 / 替换 / 删除已接入，库存目标数量调整和库存转仓已接入，销售 / 采购核心单据打印预览和 PDF 下载已接入，待处理确认已接入，完整批量盘点单尚未接入。
