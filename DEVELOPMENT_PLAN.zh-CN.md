...
阶段 7 已完成第一批高频写操作：销售 / 采购订单详情可创建下游单据、按金额登记收付款并取消订单；销售发货单、销售发票、采购收货单、采购发票详情可取消单据；销售 / 采购发票详情可取消最近收款 / 付款。后续阶段 7 仍可继续补待处理确认、更完整的下游单据参数和主数据轻量编辑。
...
- 继续增强销售 / 采购动作表单，支持部分数量、日期、备注和付款方式。
