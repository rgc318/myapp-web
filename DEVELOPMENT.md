...
- Sales delivery note detail should follow the same Ant Design Pro-style detail pattern as other sales documents: top KPI cards, a main work area for item lines and process guidance, and a side column for document properties, references, shipping information, and rollback actions.
...
- Sales invoice detail should follow the same Ant Design Pro-style detail pattern as sales order detail: top KPI cards, a main work area for payment history and item lines, and a side column for document properties, references, settlement facts, and rollback actions. Payment Entry numbers in invoice detail should link to `/payments/<payment-entry-name>`.
- Sales refund review should use submitted return Sales Invoices as the formal refund source. When a return invoice is selected, load `get_customer_refund_context_v1` for refundable amount, refunded amount, refund history, suggested refund amount, and action eligibility, then call `create_customer_refund` with refund amount, payment mode, reference number/date, and remarks to create the refund `Payment Entry`. Cancelling the latest payment entry remains a rollback action for the original receipt, not a formal customer refund.
- Sales refund review should keep the latest refund result visible after creation, refresh the refund context, show completion when the refundable amount reaches zero, present refund history with refund-specific column labels, show cumulative refunded/remaining amounts with a progress indicator, and make the return invoice/source invoice relationship explicit.
- Sales return creation should follow the same Ant Design Pro-style detail/workflow pattern as other sales pages: top KPI cards, a main work area for return item lines and submission result, and a side column for source document facts, next-step guidance, and the submit action. Use ProComponents (`ProCard`, `StatisticCard`, `ProDescriptions`, `ProTable`) before adding custom layout or raw Ant Design tables.
- Sales refund review should use the same split detail pattern: top KPI cards, a main work area for formal refund registration, refund history, and source invoice payment history, plus a side column for refund guidance, return/source invoice relationship, source invoice checks, and original-payment rollback action.
...
- Sales order detail should render `get_sales_order_detail.timeline[]` as the business timeline for sales order, delivery notes, sales invoices, payment entries, sales returns, and customer refunds. The frontend should not independently reconstruct this timeline from separate references.
- Sales order detail references and workflow links should use the backend timeline to surface payment entries, sales returns, and customer refunds. Return/refund entry points must stay disabled until the required downstream document exists, so users do not enter an empty return or refund workflow. When multiple downstream documents can be used as the source, ask the user to choose the exact document instead of silently selecting the first one.
- Payment Entry links from sales timelines, invoice details, purchase payment checks, or refund results should route to `/payments/<payment-entry-name>` so the shared payment detail page opens directly. Keep `/payments?search=<payment-entry-name>` as a list-search compatibility path.
