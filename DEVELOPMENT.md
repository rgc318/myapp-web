...
- Sales invoice detail should follow the same Ant Design Pro-style detail pattern as sales order detail: top KPI cards, a main work area for payment history and item lines, and a side column for document properties, references, settlement facts, and rollback actions. Payment Entry numbers in invoice detail should link to `/payments?search=<payment-entry-name>`.
...
- Payment Entry links from sales timelines or refund results should route to `/payments?search=<payment-entry-name>` so the shared cashflow ledger opens with the exact receipt/refund prefiltered.
