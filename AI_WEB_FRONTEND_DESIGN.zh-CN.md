...
- AI 工作台与 `/ai/drafts` 草稿中心统一复用 `AiDraftEditorModal`；销售订单、采购订单、库存调整和商品建档每次打开都重新调用草稿详情接口，保存和历史恢复必须提交当前 `expectedVersion` 并通过统一 mutation 层携带幂等键。后端返回版本冲突时保留当前输入并提示重新打开最新版本；保存成功后用新 payload、validation 和 version 刷新表单和来源卡片，避免列表行或历史消息快照覆盖人工修改。采购草稿编辑必须完整保留币种和供应商参考号。
...
1. P1：为草稿执行增加可恢复的步骤状态（已保存、已校验、执行中、回执已生成），并对版本冲突提供“对比最新版本”而不是只提示重开。
