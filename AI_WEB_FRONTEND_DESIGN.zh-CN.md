...
- `/ai/drafts` 已提供当前用户的销售订单、采购订单、库存调整和商品建档草稿中心。
...
- 每条已完成的 AI 回答提供自己的 Run 详情入口，失败回答使用同一消息级上下文打开诊断；右侧 Drawer 默认展示状态、业务场景、公司与当前账号权限范围、总耗时、消息时间和错误类别，模型、首 Token、流式分块、Token、Run 与 trace 收进“高级诊断”。工具进度、警告和可恢复动作在有数据时继续展示。
...
- 失败的助手消息保留在原消息流中，展示可理解的错误、运行诊断入口和“重新发送”；不用全局 Toast 代替可恢复状态。
- 业务数值、单据状态和草稿校验来自 citation 数据，不从 Markdown 再解析。
- 销售订单、销售发票、采购订单和采购发票查询使用 `business-result-set-v1` 聚合结果：顶部展示公司、日期、状态、排序、每类上限和权限范围，按单据类型使用 `Tabs + ProTable` 展示明细，并对 `partial / empty` 显示明确提示。
- 单据 citation 到达后立即展示结构化结果；模型尚未返回首段时状态改为“业务结果已返回，正在生成摘要”，不能继续声称首个响应尚未返回。
- 单据查询不再同时展示展开的 `Sources`、逐条 citation 卡片和重复 Markdown 清单。结构化结果是主内容，模型摘要位于结果之后且只说明范围、数量、异常和建议。
...
- 会话列表按 `active` / `archived` 查询，页面不把归档仅作为本地标记。列表筛选状态与当前打开会话的真实状态必须分离；切换列表筛选时清空当前选中会话，不能因筛选值变化误锁或误解锁会话。
- 打开历史会话时先加载最近 40 条消息，并通过后端 `before_sequence` 游标按需加载更早消息；每页完整恢复 citation、Run 摘要和已提交反馈。游标以稳定消息序号为边界，不能使用会被会话末尾新增消息扰动的 offset。
- 加载更早消息只向当前消息数组前置去重结果，并补充对应反馈；不得覆盖 Sender 中尚未发送的文本、当前 Run、流式助手占位、失败恢复上下文或已打开的消息级 Run 检查目标。前置完成后保持用户原来的阅读位置，不能强制跳到会话底部。
- 历史回答的 Run Drawer 使用该消息持久化的 Run 摘要，精确恢复状态、模型、耗时、Token 和 trace；后端未返回的历史工具过程不在前端猜测或复用其他 Run 数据。
...
- 模型选择器通过 Frappe `list_ai_selectable_models_v1` 加载当前注册表中可用的聊天模型，并保留“自动选择（策略）”默认项；Web 不根据本地配置猜测 LiteLLM 库存。用户选择固定模型后，同步 Chat、SSE、四类草稿及人工重试都携带同一 `modelAlias`，界面显示“固定模型”状态，最终 Run 仍以服务端返回的实际 `modelAlias` 为审计事实。
...
`order_query` 的 citation 首项使用 `business_result_set` 元数据，`schema_version=business-result-set-v1`；其余 citation 保留逐单据事实。Web 领域 Service 将两者合成为稳定的 `AiBusinessResultSet`，旧历史会话缺少元数据时仍可按逐单据 citation 类型恢复分组。

结构化业务结果必须标明 `queried_at`、`snapshot_source`、公司和当前账号权限范围。每个单据分组展示返回数量、可安全确定时的可见总量和 `truncated`；当后端返回 `available_count=null / truncated=null` 时，页面明确显示“可见总量未知 / 截断状态未知”，不能把“请求数量大于返回数量”解释为没有更多记录。`module_href` 提供“在业务模块查看完整结果”的次级入口。

每条已完成回答的“刷新当前数据”调用 Frappe `refresh_ai_business_result_v1`，复用该回答的受控查询范围并重新检查当前用户、公司、DocType 和记录权限。刷新不创建 Run、不调用 Orchestrator 或模型；只替换被点击消息的 `business_result_set` 和四类单据 citation，保留该消息的其他来源。刷新失败在原结果面板显示，不重新发送 Prompt。原始持久消息仍代表回答时快照，浏览器刷新会重新恢复该审计事实。

商品 citation 中的价格、库存、公司和 `queried_at` 标为“回答时数据”；`ProductDetailDrawer` 重新读取的商品、价格和分仓库存标为“当前数据”，并显示读取时间和商品最近修改时间。`BusinessDocumentDrawer` 同样并列展示结构化快照与当前领域详情，并支持不调用模型的单独刷新。
...
- 普通失败不删除助手占位；原位置保留错误、已收到内容、Run 诊断与人工重试入口，使用户能按会话时序理解失败。重新打开活跃会话时，最后一个失败 Run 应从持久化消息恢复错误、原用户问题、场景和模型选择；归档会话即使包含失败 Run 也不提供重试动作。
...
- 人工保存后由后端自动重新校验；保存成功不自动关闭编辑器，用户可在同一上下文继续校对、保存或执行。
- 打开编辑器时必须按草稿 ID 读取最新持久版本，不能使用会话消息中生成时的 citation 快照回填。校验未通过时编辑器保持打开、展示字段错误并保留本次输入。
- AI 工作台与 `/ai/drafts` 草稿中心统一复用 `AiDraftEditorModal`；销售订单、采购订单、库存调整和商品建档每次打开都重新调用草稿详情接口，保存和历史恢复必须提交当前 `expectedVersion` 并通过统一 mutation 层携带幂等键。后端以 HTTP `409` 与 `code=AI_DRAFT_VERSION_CONFLICT` 返回稳定版本冲突契约；Web 保留当前输入，展示“原打开版本 / 我的输入 / 最新持久版本”三方字段差异，并允许全部采用最新版本或在最新版本上选择性重放本地字段。订单商品明细作为整体冲突字段处理，不按数组下标静默合并。保存成功后用新 payload、validation 和 version 刷新表单和来源卡片，避免列表行或历史消息快照覆盖人工修改。采购草稿编辑必须完整保留币种和供应商参考号。
- 会话草稿卡片和草稿中心都以“复核并执行 / 完善草稿”进入共享工作台，不再在列表、Drawer 或消息卡片绕过编辑器直接执行。
- 草稿卡片和工作台业务校验步骤使用持久草稿 `modified` 显示最近一次保存并重新校验时间；不得把消息生成时间误标为草稿校验时间。
- 会话草稿卡片与草稿中心列表共用 `AiDraftCompactSummary`。摘要只消费草稿持久化 payload：商品展示名称、库存单位、四档价格、初始库存和仓库；销售/采购展示往来单位、日期、行数、按 UOM 分组数量、草稿金额和仓库；库存调整展示商品、仓库、当前/目标/差异库存、估值参考和原因。订单任一行缺少数量或价格时不展示误导性的部分金额合计。
- 用户点击“确认执行”时，如有未保存修改则先保存并取得新版本；如无修改则重读后端最新版本。只能使用该最新持久版本通过校验并执行，不得使用初始 citation 中的旧版本号。
- 执行前必须在当前编辑器上方展示业务复核摘要和明确版本号；执行中锁定关闭、保存和重复提交，执行后原地展示正式业务对象、请求号、执行人和执行时间等回执。
- 历史恢复不会覆盖当前版本，而是由后端创建新版本。
- 放弃草稿后禁用编辑和交接。
- 默认操作在 AI 工作台或草稿中心原地确认执行，正式保存仍复用既有业务接口、权限、幂等和审计。一次性 `sessionStorage` 交接保留为复杂场景的次级入口。
...
- 模型注册同步展示当前 LiteLLM Key 可见的完整库存；普通工作台的可选列表只包含 `active / validated` 的聊天能力模型，不包含 Embedding、缺失、停用或退役模型。
- 修改治理元数据需要后端权限、原因和审计。
...
| AI Model Manager | 查看模型管理，维护模型元数据和策略草稿 |
...
- 对话消息区独立滚动，避免整页重排；长会话首次只读取最近 40 条，按消息序号游标向前增量加载并保持原阅读位置。
...
1. P1（已实现，待 staging 人工验收）：草稿工作台展示保存、校验、执行和回执步骤状态；版本冲突提供三方字段对比、选择性重放和全部采用最新版本。
2. P1（已实现，待 staging 人工验收）：失败消息按稳定错误码区分可重试、需修改输入、权限拒绝和系统/治理故障，并给出差异化恢复动作。
3. P1（已实现，待 staging 人工验收）：会话草稿卡片与草稿中心展示共享紧凑业务摘要，订单数量按 UOM 分组且金额不使用不完整行做部分合计。
4. P1（已实现，待 staging 人工验收）：每条回答独立打开自己的 Run；顶部仅在生成中显示“当前运行”，普通概览与高级诊断分层展示。
5. P1（已实现，待 staging 人工验收）：超长会话使用最近 40 条首屏和稳定序号游标向前加载；历史 Run、citation、反馈、失败消息和草稿入口随消息页恢复，加载过程不覆盖 Sender 或当前运行状态。
6. P2：建立对话与草稿的快捷键、焦点恢复和屏幕阅读器验收矩阵，并补齐草稿字段级修改证据。
7. P2：对 AI 相关大依赖继续分析 chunk，评估按能力延迟加载，并建立首屏、首 Token、长会话内存和交互响应性预算。
8. P2：为高频治理查询增加可控缓存和刷新策略。
