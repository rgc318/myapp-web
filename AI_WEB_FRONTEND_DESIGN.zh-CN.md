...
3. P1：为超长会话增加分页或窗口化渲染，同时保留 Run、citation、反馈和失败状态的可达性。
4. P2：建立对话与草稿的快捷键、焦点恢复和屏幕阅读器验收矩阵，并补齐草稿字段级修改证据。
5. P2：对 AI 相关大依赖继续分析 chunk，评估按能力延迟加载，并建立首屏、首 Token、长会话内存和交互响应性预算。
6. P2：为高频治理查询增加可控缓存和刷新策略。
