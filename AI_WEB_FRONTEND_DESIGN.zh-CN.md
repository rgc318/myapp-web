...
- 页面展示 `lastHealthAt`、`available / unavailable`、工具能力和稳定错误码。健康是时间快照；不得把旧的绿色状态表述为当前 Provider SLA。
