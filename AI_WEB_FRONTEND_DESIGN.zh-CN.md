...
商品完善草稿必须明确展示已绑定的商品编码。未唯一绑定时，编辑器显示基于 `RemoteLinkSelect` 的“选择现有商品”，允许按编码或名称搜索；选择并保存后由 Backend 重新读取商品、校验权限与版本并写入 `_state.entity`，随后编码改为只读。已绑定商品的库存基准单位在普通完善流程中锁定，不能借商品字段保存绕过 ERPNext 历史库存约束。不得出现“要求补商品编码但字段被禁用”的不可恢复状态。
...
销售、采购、库存调整和商品创建/完善草稿使用严格结构化模型结果，并在 Frappe 侧重新做权限、主数据、UOM、价格或库存校验，因此不能把未闭合 JSON 当作业务草稿逐字展示。商品创建模式展示主数据、四类价格和初始库存候选；完善模式显示现有 baseline 到用户 patch 的变化，商品编码不可编辑，隐藏初始库存字段，并把公司当前库存标为只读上下文。缺失价格显示为未设置，明确零价显示为 0，不能互相替代。
...
- 会话草稿卡片与草稿中心列表共用 `AiDraftCompactSummary`。摘要只消费草稿持久化 payload：新商品展示名称、库存单位、四档价格、初始库存和仓库；完善现有商品展示已水合的当前价格和只读当前库存。销售/采购展示往来单位、日期、行数、按 UOM 分组数量、草稿金额和仓库；库存调整展示商品、仓库、当前/目标/差异库存、估值参考和原因。订单任一行缺少数量或价格时不展示误导性的部分金额合计。
