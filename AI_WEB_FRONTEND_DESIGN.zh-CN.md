...
商品 citation 中的图片、价格、库存、公司和 `queried_at` 标为“回答时数据”；citation 卡片显示小图，`ProductDetailDrawer` 重新读取并展示当前商品图片、价格和分仓库存，标为“当前数据”，同时显示读取时间和商品最近修改时间。`BusinessDocumentDrawer` 的当前单据明细同样展示商品图片与规格，并列展示结构化快照与当前领域详情，支持不调用模型的单独刷新。
