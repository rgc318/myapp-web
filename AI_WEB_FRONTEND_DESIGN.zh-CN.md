...
销售、采购、库存调整和商品建档草稿使用严格结构化模型结果，并在 Frappe 侧重新做权限、主数据、UOM、价格或库存校验，因此不能把未闭合 JSON 当作业务草稿逐字展示。商品建档同时展示主数据、标准售价、默认采购价和初始库存候选；初始库存使用库存基准单位，默认采购价作为首次入库成本，标准售价不得用于库存计价。页面在等待期间显示“结构化生成与后端校验”阶段，只有最终验证通过或带明确校验错误的草稿才整体呈现。
...
- 人工保存后由后端自动重新校验；界面主操作使用“完善草稿 / 编辑草稿”和“保存草稿”，不把技术过程写进按钮文案。
