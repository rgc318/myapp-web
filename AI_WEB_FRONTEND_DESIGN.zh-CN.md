...
## 商品启停与删除计划（2026-09-07）

预检阻断的视觉语义：顶部使用红色“预检未通过”标签，逐行使用红色“不允许执行”标签及红色加粗原因；整批不通过使用带错误图标的红色 Alert，权限/执行异常原因同样标红。一般共享范围风险仍使用黄色警告，正常通过不标红；不改变后端计划状态或执行规则。

自动场景新增 `product_lifecycle_plan`，调用独立领域 service，不走编辑草稿或 Chat 执行。AI 只生成待确认计划；聊天内计划卡片和顶部“商品操作计划”均可重新打开服务器保存的预览，列表展示最近 50 条个人计划。多候选须逐项选择后重新预检；明确保留对象单独展示，任何阻断均禁止整批执行。

`ProductLifecyclePlanModal` 使用 Ant Design Modal/Table/Select/Checkbox：展示原文、编码、原状态、共享范围、版本、有效期及阻断原因；启停两个确认，删除三个确认，初始均不勾选。有效期采用服务端剩余秒数，打开/刷新/替换计划重置确认。执行单飞、禁止关闭，失败显示原因；可刷新或用固定计划幂等键重试，不自动重试写操作。完成后显示服务器持久回执，刷新聊天可恢复计划卡片。

权限与执行规则全部由 Backend 决定；组件不自行判断商品“零库存可删除”，也不从候选中默认选第一项。商品资料常规入口本次未另建删除逻辑。已有引用、图片和附件会保守阻断删除，不能承诺一键撤销。组件和页面回归验证确认、过期、阻断、重开状态以及删除不回退 Chat/编辑；HTTP 联调不等于浏览器端到端验收。
...
- 主区：品牌栏、当前场景/公司上下文、页头快速模型选择、消息列表、Welcome/Prompts 空态和浮动 Sender 输入区。完整场景和模型说明仍保留在“高级设置” Drawer；具备固定模型权限的账号可在页头直接切换模型，不需要为日常重试反复打开 Drawer。
...
- AI 草稿 citation 先由领域 Service 规范化为 `AiDraft`，再由共享紧凑摘要组件展示；商品建档展示正式名称、昵称、库存单位、四档价格和初始库存，销售/采购按单位分别汇总数量并展示完整行金额合计，库存调整展示当前、目标、差异、估值参考和原因。商品昵称是辅助识别字段，不能取代正式名称。不得把不同 UOM 的数量直接相加。
- AI 草稿中的商品搜索词未唯一解析时，后端返回的权限内 `candidates` 必须直接预载到商品选择器，并继续复用正式商品领域搜索支持编码、名称、昵称、条码和规格查询；不得改走只按 DocType 名称搜索的通用 Link 下拉。多候选必须展示商品编码与名称供用户明确选择；只有一个模糊或语义候选时也必须提示用户确认，不能显示为“唯一匹配”或自动绑定。只有编码、条码或真正唯一的精确名称可以自动解析。搜索接口失败时展示真实错误，不能伪装成“暂无数据”。库存商品尚未唯一选择时，紧凑摘要仍展示模型已提取的增减方式、原始数量和单位，并明确说明目标库存待选择商品后计算。
...
本地活跃请求是实时消息的唯一更新来源，持久恢复轮询在此期间暂停，并在异步响应落地前再次检查请求归属。不能用尚未落库的空消息覆盖已到达引用或增量正文。无本地请求的历史运行仍允许轮询恢复。模型按轮次选择：新一轮可切换合格模型并继承同一会话；生成中不更换当前 Run，审批恢复仍沿用原任务模型。
...
- 会话侧栏使用服务端关键词搜索，匹配本人会话标题和消息正文；列表展示最近消息时间、消息数、公司和待复核草稿数。“我的草稿”入口展示当前用户全部 `draft` 状态草稿 Badge，草稿执行或放弃后刷新统计。
- 活跃和归档会话都允许本人重命名；重命名只修改会话主档标题，不改变公司、状态、消息审计事实或最近消息排序。归档动作仍只对活跃会话开放。
- Sender 未发送文本按会话 ID 保存在当前浏览器内存中；切换会话、切换活跃/归档列表或临时进入新会话时分别恢复。提交成功开始请求后清空该会话文本，归档时清除对应未发送文本；不得把一个会话的输入带到另一个会话。
...
- 历史回答的 Run Drawer 使用该消息持久化的 Run 摘要，精确恢复状态与耗时；具备高级诊断权限时再恢复模型、Token、首 Token、流式统计和 trace。后端未返回的诊断或历史工具过程不在前端猜测或复用其他 Run 数据。
- URL 可携带 `conversation` 参数，用于从草稿或其他业务入口回到来源会话。
- 页面刷新后不依赖内存中的 `lastResult` 恢复 Run 和反馈状态；技术诊断字段是否存在完全服从后端能力位和脱敏响应。
- 页面刷新或重新打开会话时，以 `get_ai_conversation_v1.latest_run` 恢复最新持久 Run；不能把所有非失败状态折叠为“已完成”。只有 `latest_run.message_id` 为空且当前消息页没有该 Run 时才补临时助手占位，避免把分页之外已经存在的旧回答错误追加到会话末尾。`running` 和 `waiting_approval` 每 3 秒低频读取一次持久快照，直到收敛为 `completed / failed / expired / cancelled`，轮询失败保留最后已知状态且不伪造终态。
...
- 活跃会话显示服务端受控工作上下文状态，并提供“清除上下文”确认操作。清除后历史消息、Run、citation 和草稿仍可查看，但后续模型消息窗口从会话当前末尾重新开始，商品、业务单据、客户/供应商 typed entity，订单筛选、报表口径和结果集引用不再继承。归档会话不允许修改上下文。
- 工作上下文过期时间完全以后端返回为准；页面只显示 `active / empty / expired / invalid` 状态，不在浏览器自行计算或延长 TTL。状态过期或损坏后，下一次发送由后端建立新上下文边界并记录审计原因。
- 商品查询结果卡片同时提供“当前页查看”“编辑商品资料”和“调整库存”。后两个按钮调用 Frappe 确定性草稿接口并直接打开共享草稿编辑器，不填充 Sender、不要求再次发送，也不产生模型调用。用户直接说“修改这个商品”时，Backend 仍只从当前有效会话中唯一解析的商品或唯一商品结果集继承目标；多个结果不得由 Web 或模型猜测。
- 未唯一匹配商品的库存草稿直接显示后端候选按钮。用户点击候选，或下一条纯文本能在当前候选中唯一匹配时，Web 调用 `select_ai_draft_product_candidate_v1` 续接同一个草稿并打开编辑器；多个匹配必须保留原草稿并要求明确选择，不得把文字重新路由成新的商品搜索。
...
商品 citation 中的图片、价格、库存、公司和 `queried_at` 标为“回答时数据”；citation 卡片始终保留图片位置，无图或资源加载失败时显示统一占位。`ProductDetailDrawer` 重新读取并展示当前商品图片、价格和分仓库存，标为“当前数据”，同时显示读取时间和商品最近修改时间；具备后端 Item 写权限的用户可在该 Drawer 直接上传、替换或删除当前商品图片，成功后重新读取正式数据，不调用模型。`BusinessDocumentDrawer` 的当前单据明细同样展示商品图片与规格，并列展示结构化快照与当前领域详情，支持不调用模型的单独刷新。

商品完善草稿必须明确展示已绑定的商品编码。未唯一绑定时，编辑器显示基于 `RemoteLinkSelect` 的“选择现有商品”，允许按编码、正式名称、昵称、条码或规格搜索；候选在正式名称和编码之外辅助展示昵称，选择并保存后由 Backend 重新读取商品、校验权限与版本并写入 `_state.entity`，随后编码改为只读。已绑定商品的库存基准单位在普通完善流程中锁定，不能借商品字段保存绕过 ERPNext 历史库存约束。不得出现“要求补商品编码但字段被禁用”的不可恢复状态。

商品完善编辑器顶部展示公司总库存、分仓库存和库存基准单位，并提供“调整此商品库存”入口。库存仍是独立交易草稿，不随商品主数据一起保存；若 Backend 标记 `requires_uom_migration=true`，入口改为“处理单位异常”，直接进入商品详情的受控单位错误迁移流程。Modal body 必须限制在当前视口内并内部滚动，标题、库存摘要、表单和 footer 均可访问。

浏览器使用 `fetch + ReadableStream`，不使用原生 `EventSource`，因为请求需要 POST body 和 JWT Bearer Header。

销售、采购、库存调整和商品创建/完善草稿使用严格结构化模型结果，并在 Frappe 侧重新做权限、主数据、UOM、价格或库存校验，因此不能把未闭合 JSON 当作业务草稿逐字展示。商品创建模式展示主数据（含昵称）、四类价格和初始库存候选；完善模式显示现有 baseline 到用户 patch 的变化，昵称的新增、修改和清空都必须进入草稿版本、差异、摘要与最终复核，商品编码不可编辑，隐藏初始库存字段，并把公司当前库存标为只读上下文。缺失价格显示为未设置，明确零价显示为 0，不能互相替代。
...
- 普通失败不删除助手占位；原位置保留错误、已收到内容、Run 诊断与“使用当前模型重试”入口，使用户能按会话时序理解失败。重新打开活跃会话时，最后一个失败 Run 应从持久化消息恢复错误、原用户问题和场景；重试模型只取页头当前选择，不恢复失败时的旧选择。归档会话即使包含失败 Run 也不提供重试动作。
- 普通聊天和四类草稿重试都发送失败消息的 `retryRunId`，不重复插入用户气泡，并用新 Run 原位替换失败助手消息。草稿场景继续调用对应草稿生成接口，由 Backend 从失败 Run 恢复原文字、公司、会话和附件，不能让浏览器重新拼装证据。旧 Run 只用于审计，不能被覆盖或改写成成功。
...
编辑器消费领域 service 的 `boundFields`，对后端已绑定的动作和目标范围禁用切换；不按提示文案或普通 payload 推断锁定。未解析字段可首次确认，正常数量/金额可编辑；新建不能通过重复候选切换为修改。保存、执行和版本冲突锁定优先于单字段可编辑状态，后端始终重校验。

共享编辑器从领域 service 的 `boundScopeSummary` 展示持久化已绑定范围（目标、仓库、库存方向），说明更换范围需回到对话重新生成。没有服务端快照时不推断绑定；正常业务字段继续可编辑，后端仍是范围校验权威。提示不代表模型首次解析已被证明正确，用户执行前仍需核对原意。
...
- 商品草稿的图片上传使用暂存模式：选择或删除图片只更新当前表单和草稿 payload，不直接调用正式商品替换/删除接口。用户保存草稿后图片 URL 作为结构化字段进入版本、差异和复核摘要；只有确认执行时才随 `create_product_v2` / `update_product_v2` 原子绑定或删除正式 `Item.image`。用户取消编辑不会提前改变正式商品。
- 图片不进入模型消息或 Prompt。模型只提取商品建档意图和文本字段；图片由用户在 Web 端选择，经 Frappe 受控上传后进入草稿。仍被 `draft` 或 `handed_off` 草稿引用的暂存图片受清理任务保护，复杂场景交接到商品模块时必须继续携带并预览该图片。
- 商品草稿 citation、紧凑摘要和业务复核始终展示真实图片或“未设置”占位，避免无图时用户误判图片能力未部署。现有商品的独立图片维护不经过草稿 payload，必须在当前商品详情中明确提示其为立即保存动作，并继续由 Frappe Item 权限裁决。
- 打开编辑器时必须按草稿 ID 读取最新持久版本，不能使用会话消息中生成时的 citation 快照回填。校验未通过时编辑器保持打开、展示字段错误并保留本次输入。
- 未解析的客户、供应商、商品、商品分类、品牌或仓库查询词只能作为搜索提示和首次远程检索条件，不能回填成已选 Link 值。Web 应把后端校验错误绑定到对应字段或订单行，并在人工只修改其他字段时继续带回原始 `*_query`，直到用户选择具体主键或后端重新唯一解析；点击执行时必须完整展示阻断原因并定位首个可操作字段。
...
- 会话草稿卡片与草稿中心列表共用 `AiDraftCompactSummary`。摘要只消费草稿持久化 payload：新商品展示正式名称、昵称、库存单位、四档价格、初始库存和仓库；完善现有商品展示昵称变化、已水合的当前价格和只读当前库存。销售/采购展示往来单位、日期、行数、按 UOM 分组数量、草稿金额和仓库；库存调整展示商品、仓库、当前/目标/差异库存、估值参考和原因。订单任一行缺少数量或价格时不展示误导性的部分金额合计。
- 草稿保存必须原样带回后端 `_state`。订单行系统参考价与人工覆盖价分别维护；确认执行后若主数据、系统价、UOM 换算或实时库存变化，后端会生成新版本并返回 `AI_DRAFT_VERSION_CONFLICT`，Web 复用现有三方冲突界面要求用户重新确认，不能静默使用旧快照执行。
...
- `MODEL_PROVIDER_REJECTED` 显示“本次模型不可用”，并显示服务端返回的 `modelDisplay`。
- 自动模式同样显示本次实际模型；不能用“自动选择”替代 Run 事实。
- 具备高级诊断权限时额外显示 `modelAlias` 和 `providerErrorCode`；普通用户只显示友好名称和安全建议。
- Provider 拒绝只提供人工重试和更换模型入口，不自动再次产生模型调用；点击重试时使用点击瞬间的页头模型。
- Run Inspector 分别展示“请求方式、请求模型、实际模型”；高级诊断再展示请求 alias 和实际 alias。助手消息、失败卡和历史 Run 使用同一组服务端字段，避免页面不同区域分别推断模型。
...
- 普通 AI 工作区使用低密度、低不透明度水印，保证长对话可读；审计、用户管理、打印预览与打印历史使用更强水印。水印开关仍服从构建环境配置，不能用 CSS 隐藏代替权限或审计控制。
...
当前阶段以“推送、staging 最小冒烟、发现真实问题后针对性修复”为准，不继续主动扩大工程范围。下面已实现的 P1 项只需完成人工验收；P2 可达性矩阵、性能预算、治理缓存、完整 E2E、共享编排重写和大型组件拆分均暂缓，不作为近期发布前置条件。只有出现可量化的回归成本、性能问题、管理效率问题或发布风险时，才单独立项。
...
6. P2（暂缓）：建立对话与草稿的快捷键、焦点恢复和屏幕阅读器验收矩阵，并补齐草稿字段级修改证据。
7. P2（暂缓）：对 AI 相关大依赖继续分析 chunk，评估按能力延迟加载，并建立首屏、首 Token、长会话内存和交互响应性预算。
8. P2（暂缓）：为高频治理查询增加可控缓存和刷新策略。
...
- 待发送附件与未发送文本一样按会话键保存在当前浏览器内存。上传队列开始时固定目标会话，用户在上传期间切换会话后，结果只能写回原会话；重新打开会话时恢复对应附件，不能把 A 会话待发送图片带到 B 会话。
- 自动场景识别会把附件交给视觉意图模型。商品实物/包装可进入商品建档草稿，严格订单表格可进入销售或采购草稿；不确定时保持通用对话。
- 固定模型 `supportsVision=false` 时发送前阻止；检查范围同时覆盖本次待发送图片和当前有效模型上下文窗口中的历史图片。自动模式由 Runtime Policy 选择已经通过视觉探测的模型。
- 失败重试保存原请求是否带图。点击“使用当前模型重试”时，附件 ID 由 Backend 从失败 Run 恢复；Web 只用该模态事实检查当前固定模型，若不支持视觉则在请求前阻止，不能因为重试没有本地待发送附件而绕过检查。
- 商品草稿展示新增/完善选择和疑似重复候选。新增时来源首图可成为暂存封面；完善现有商品时来源图不进入图片字段。编辑器支持条码和规格。
- 订单草稿展示创建/修改和来源订单号；只有原草稿已明确替换明细或用户实际改过明细时才提交 `update_items_explicit=true`，只改备注不会触发明细替换。修改执行仍复用后端正式订单更新服务，并由 Backend 校验来源订单版本。空数量、单位、规格或价格保持待补充，不由前端填默认值掩盖。
# 逐单位商品价格增量（2026-09-09）

提交前复验：`npm run tsc`、`npm run biome:lint`、全量 Jest 61 suites / 411 tests 通过。测试覆盖明细 DTO 往返、金额/单位和推断来源展示、缺包装数量编辑，以及既有聊天/业务组件回归；不替代真实浏览器端到端。Jest 仍有既有 open-handle 提示，退出码0。用户本地 `public/scripts/loading.js` 改动未纳入本功能提交。

`product-pricing-v1` 由领域适配器 `ai-product-pricing.ts` 映射。AI 草稿摘要、详情与共享编辑器展示价格表、金额/计价单位、推断/默认来源；编辑支持多行价格、明确数量等式、批零默认单位。换算只由后端共享 UOM 领域计算。未补齐数量时保留草稿并显示错误，不能执行。标准默认价后端随批发价更新，用户明确编辑则成为 user，不再被覆盖。

新契约隐藏旧“在业务编辑器继续”入口（旧商品交接页无法完整保留明细），使用“复核并执行”。原标量草稿保留旧显示路径，带价格旧模型草稿由后端要求重新生成。移除草稿行不是删除已有正式价格；后端阻止此类误用。阶梯/客户价/有效期等暂不支持，不得以普通单价替代。
