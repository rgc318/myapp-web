...
- 商品查询结果卡片同时提供“当前页查看”和“完善此商品”。查看只打开当前商品详情，不暗示写操作选择；“完善此商品”把受控商品编码预填到 Sender 并将本次场景设为商品草稿，用户补充修改内容后再显式发送。用户直接说“修改这个商品”时，Backend 只从当前有效会话中唯一解析的商品或唯一商品结果集继承目标；多个结果不得由 Web 或模型猜测。
...
- 自动识别在发送前调用 Frappe `resolve_ai_scenario_v1`，并始终携带当前公司和会话 ID，使结构化语义路由能读取服务端 `conversation-state-v2`。Web 只采用四类写草稿结果进入专用校验链路；识别为 `general / product_search / order_query / report_summary` 时，普通 Chat 请求仍发送 `scenario=auto`，由 Backend 的结构化意图模型决定只读工具和参数。Web 不解析“这个商品/订单/客户/供应商”，不把粗粒度规则结果锁成最终只读场景，也不复制或扩展关键词规则。历史消息中的 `scenario` 是审计事实而不是持久 UI 偏好，重新打开会话后场景选择器恢复 `auto`，防止上一轮订单查询污染后续商品建档等不同意图。
...
商品完善草稿必须明确展示已绑定的商品编码。未唯一绑定时，编辑器显示基于 `RemoteLinkSelect` 的“选择现有商品”，允许按编码或名称搜索；选择并保存后由 Backend 重新读取商品、校验权限与版本并写入 `_state.entity`，随后编码改为只读。不得出现“要求补商品编码但字段被禁用”的不可恢复状态。
