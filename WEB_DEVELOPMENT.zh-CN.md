...
商品详情页的库存基准单位继续在普通编辑中锁定。若商品库存单位错误，使用“单位错误纠正”入口并遵循双策略：没有历史库存流水、库存/占用/未完订单均为零且没有既有替代关系时，可保留原编码原地纠正；存在历史库存流水或商品物理身份/包装基础真正变化时，创建继任商品并停用源商品。向导必须展示后端推荐与原因、要求填写纠正原因，并逐条确认价格/条码单位处理。价格可以保留原金额、手工指定新金额、跳过，或新增独立价格；错误旧单位没有可信关系时不得自动换算。条码不是必填，空集合按空数组提交。执行前必须展示最终变更预览。

普通商品编辑不能因为用户只改图片、描述或其他字段就重写价格。编辑弹窗每次打开先清除旧 touched 状态；四个汇总价格字段只有本次确实被用户修改时才提交，零价不作为默认输入值，从而避免自动补出重复单位价格或新的 0 元价格。商品详情切换到继任商品后，新增条码表单必须同步重置到继任商品库存单位，不能残留旧异常单位。
