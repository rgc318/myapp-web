...
- 六类单据列表均已接入批量选择和 `PrintBatchAction`：创建异步 PDF 批次后通过 Drawer 查询进度，支持取消、失败项重试和成功项 ZIP 下载；单批前后端均限制最多 100 张单据。
- 批次处于 `queued`、`processing`、`cancel_requested` 时自动轮询，进入 `completed`、`partial_failed`、`failed`、`canceled` 后停止。
- `/printing/settings` 为 `System Manager` 打印设置页，合并 `list_print_doctypes_v1` 和 `get_print_settings_v1` 展示 6 类单据，并通过 `set_print_default_template_v1` 维护全局默认模板。
