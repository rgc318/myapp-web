...
价格表名称是 Item Price、客户/供应商默认价格表和交易单据引用的稳定标识，不为界面翻译直接重命名。商品价格界面通过共享显示映射本地化内置 `Standard Selling / Standard Buying / Wholesale / Retail`，提交接口、记录定位与审计仍使用原始标识；自定义价格表名称保持原样，选择器可同时展示本地化名称和原始标识。
