...
- 交易选品 Drawer 右侧应常驻展示“本单已加入”和“本次暂存”：本单已加入用于解释订单当前已有业务行和同 SKU 多行来源，本次暂存用于批量勾选后修改数量、移除和统一加入；每行业务信息必须展示数量 + 单位、金额、仓库、模式、单价，并使用共享 UOM / 换算 / 金额工具生成展示值。不要要求用户再打开二级抽屉才能确认已选内容。非交易类弹窗选品可继续使用“已选明细”抽屉展示暂存业务行。
