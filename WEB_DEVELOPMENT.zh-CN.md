...
商品详情页的库存基准单位继续在普通编辑中锁定。若历史商品库存单位错误，必须使用“单位错误迁移”入口：先读取后端评估报告，再明确填写新商品编码、正确库存单位和换算表，并逐条确认价格/条码单位映射。Web 不得直接覆盖旧 `stock_uom`，不得预选价格或条码迁移动作，也不得在存在库存、库存占用或未完订单时允许提交。
