...
商品单位表只锁定第一条正式库存基准单位行及其固定换算系数 `1`。若用户把其他行误选成相同单位，重复行必须保持可编辑、可删除，并显示“同一单位不能重复配置”校验，避免错误状态反过来锁死纠正入口。

商品生命周期闭环：AI 工作台顶部“商品操作计划”与聊天计划卡片使用独立 `product-lifecycle` 领域 service。启停/删除不复用商品编辑草稿，删除须全部目标、共享范围、删除风险三项显式确认；模糊目标逐项选择、重新预检，阻断计划不可部分执行。每次打开读取持久状态，固定计划幂等键支持不确定响应后的恢复。开发与测试细节见 `AI_WEB_FRONTEND_DESIGN.zh-CN.md` 的商品启停与删除计划章节。

实时消息与恢复轮询必须互斥：本地发送请求存续期间，由 SSE 更新正文、引用、工具进度和最终结果，不启动会话恢复轮询；轮询响应返回后也检查本地请求是否已启动，避免旧快照覆盖实时卡片/文字。恢复历史 running/waiting_approval Run 时仍每 3 秒读取持久状态。模型选择按每次发送取值，前置 resolve 与后续 stream 使用同一 modelAlias；同会话后续轮次可换模型，不改写历史回答的模型事实。

范围锁定后续增量：`ai.ts` 从持久契约派生 `boundFields`，共享编辑器禁用已绑定的处理方式、商品/订单目标、库存仓库和调整方式。未绑定目标允许首次选择；新建契约禁止借疑似重复候选切换为修改，修改契约的未解析目标仍可选择候选。数量、金额等普通字段保留业务编辑，保存/执行中和版本冲突时仍遵循全局锁定。前端禁用只改善体验，不替代后端权限、契约和版本校验；下面“未全面禁用”为初始提示阶段边界。

共享 AI 草稿编辑器展示最新持久版本的已绑定目标、仓库和库存调整方向，提示更换范围需返回对话重新生成，并提醒执行前核对是否符合原意。`ai.ts` 将服务端 `resolved_scope` 映射为 `boundScopeSummary`，页面不读取私有契约或从普通 payload 推断绑定。缺少绑定快照时不展示该提示；此轮仅提供范围说明，不以 UI 替代后端约束，也未全面禁用相关表单字段。
...
现有商品统一进入 `/master-data/products/:itemCode/edit` 全屏维护工作区，不再复用新增商品弹窗。工作区按基本资料、单位与包装、销售价格、采购价格、条码、库存与估值、变更历史分区；普通资料、单位默认值和估值可以在同一工作区维护，正式价目表仍按价格记录独立保存，避免一次普通资料提交静默重写价格。

商品昵称是内部识别字段，不替代正式商品名称。商品列表和所有远程选品候选在正式名称旁辅助展示昵称，关键词搜索提示明确包含昵称；新增、全屏维护、清空保存、CSV 导入导出及 AI 商品建档/完善草稿都必须携带 `nickname`，避免只读详情可见而主维护链路无法编辑。销售、采购、库存和 AI 目标选择统一走商品领域搜索，搜索字段必须包含昵称。

库存基准单位在工作区中继续锁定。若商品库存单位错误，使用“单位风险纠正”入口并遵循双策略：没有历史库存流水、库存/占用/未完订单均为零且没有既有替代关系时，可保留原编码原地纠正；存在历史库存流水或商品物理身份/包装基础真正变化时，创建继任商品并停用源商品。向导必须展示后端推荐与原因、要求填写纠正原因，并逐条确认价格和条码动作。价格动作必须明确为保持金额、重新定价、终止旧价格或不迁移，不得用含糊的“跳过”代替；错误旧单位没有可信关系时不得自动换算。条码不是必填，空集合按空数组提交。执行前必须展示最终变更预览。

若 `assessment.canExecuteWithInventoryConversion = true`，说明唯一 blocker 是正库存，Web 允许继续选择继任商品策略，并展示逐仓“正式 Repack”确认区。每个正库存仓库都必须由用户填写盘点确认后的继任商品数量；旧数量只读展示，前端不得根据旧抽象单位或新换算表自动计算目标数量。提交时同时发送 `confirmInventoryConversion` 和完整 `inventoryMappings`，预览必须明确逐仓转出/转入数量、原子回滚语义及历史流水仍保留在源商品下。负库存、占用、未完销售/采购订单等其他 blocker 仍禁用确认按钮，并直接展示不能在向导内解除的原因。

工作区必须提示未保存状态，并在刷新浏览器、返回详情或切换页签前保护尚未保存的资料。条码和正式价格使用各自的即时写接口，不混入普通资料表单；商品保存、条码动作和价格动作分别反馈结果。商品详情切换到继任商品后，新增条码表单必须同步重置到继任商品库存单位，不能残留旧异常单位。

商品 CSV 导入采用“解析校验 → 更新行预检 → 逐行执行 → 可恢复结果”的治理流程。更新行在执行前必须读取目标商品、验证写权限并保存当前 `modified`，执行时携带该版本；版本冲突失败关闭，重新读取最新版本后才能重试。每行使用稳定幂等键，成功行进入终态并在后续点击中自动跳过，失败行不回滚其他成功记录。CSV 中未知动作、非法数字和非法启停值必须在本地标记为格式错误，不能猜测为新增或静默忽略。批量过程只展示汇总与逐行结果，不逐条弹出成功或错误通知。

商品 CSV 导出按当前筛选快照分页读取，不得把页面请求的 `limit=1000` 误认为后端会返回 1000 行。当前后端单页最多 100 行，Web 使用商品编码升序和有界并发拉取全部分页；分页期间总数变化、缺行或重复时不生成不可信文件。浏览器同步导出上限为 5000 行，超限时明确阻断并要求缩小筛选，后续大数据量场景使用后端异步导出任务。导出内容同时保留条码单位、库存单位编码与显示名、换算关系、批发/零售默认单位、价格摘要、最后修改时间和筛选口径。

商品“资料质量”不使用前端随意扣分的伪精确百分比，也不把条码、品牌、图片、标准售价或标准采购价固定定义为建档必填。质量评估分为数据错误、业务风险和可选完善建议：库存基准单位/换算错误、默认单位或价格/条码引用未配置单位属于数据错误；负库存、停用仍有库存、历史条码或价格缺少单位属于业务风险；条码、品牌、图片、描述和参考价缺失只作为可选建议。商品列表展示良好/需关注/异常的紧凑状态，详情页展示分级明细；每条问题必须跳转到对应的单位、条码、销售价格、采购价格、库存或基础资料区，价格维护权限继续与 Item 普通写权限分离。

价格表名称是 Item Price、客户/供应商默认价格表和交易单据引用的稳定标识，不为界面翻译直接重命名。所有面向用户的价格表只读展示统一使用共享 `PriceListName`，本地化内置 `Standard Selling / Standard Buying / Wholesale / Retail` 并禁止窄表格逐字换行；选择器同时展示本地化名称和原始标识。提交接口、记录定位、审计源、CSV 导入导出和自定义价格表仍保留原始稳定标识，不得把本地化文字写回业务字段。
...
销售 / 采购订单页面可以共享选品器、明细表、金额摘要、关联单据链接、业务时间线和收付款表列，但不要把销售 / 采购状态机、接口编排和回退规则强行抽成一套通用流程。

采购订单的“默认取值模式”是前端选品辅助字段：

- 默认值为 `wholesale`，用于商品加入采购明细时优先取商品批发档案默认单位。
- 切换为 `retail` 时，只影响后续选品和当前草稿行的默认单位。
- 采购价仍按采购参考价 / 标准采购价取值，不使用销售价。
- 该字段不提交给采购订单后端接口，也不作为采购订单持久化字段。

交易仓库选择遵循同一规则：

- 前端默认仓库、行仓库、选品器库存仓库和快捷建品入库仓库都应过滤 `disabled=0`、`is_group=0`。
- 父级 / 汇总仓只用于仓库树和报表汇总，不应出现在订单明细可选项。
- 前端过滤只负责体验；后端写接口仍必须校验仓库存在、未停用、`is_group=0` 且公司匹配。

查询页面的公司筛选遵循以下规则：

- 工作区“默认公司”只用于新建订单、库存作业、仓库新增和 AI 草稿等录入上下文，不得自动写入列表、搜索、报表或 Dashboard 的公司筛选。
- 查询页首次加载时公司条件必须为空，表示查询当前用户权限范围内的全部公司；只有用户在筛选栏主动选择公司后，前端才向领域 service 传递 `company`。
- 不得通过 `initialValue: defaultCompany`、以 `defaultCompany` 作为 ProTable `key`，或偏好加载后的 `useEffect` 二次覆盖查询条件；这些写法会造成首次全量结果被第二次公司限定请求覆盖。
- 商品/库存详情只在 URL 明确携带 `company` 时按公司查询，不从工作区偏好隐式补公司。
...
- 管理入口为 `/administration/test-data`，仅 `canAdmin` 用户可见。
- 页面领域 service 为 `src/services/myapp/test-data.ts`，负责独立 Frappe 方法包络和蛇形字段映射。
- 页面必须先调用 preview，再允许首次生成、补充场景或完整重置；执行前必须输入后端返回的动态确认文本。
- 补充模式通过多选场景代码追加交易数据，复用登记拥有的标准客户、供应商、商品与库存基线，不重复创建主数据。
- 数据量档位使用 `small`、`medium`、`large`，分别生成每个选定场景的 1、5、20 份实例；页面必须展示份数、场景实例总数和预计对象总数，并将 `scale` 同时传给 preview 与运行请求。
- 公司与仓库使用 `RemoteLinkSelect`，仓库必须按公司、`disabled=0`、`is_group=0` 过滤。
- 后台执行后页面轮询运行状态和 Redis 实时进度，并显示审计历史、失败日志和完整性验证结果。
- 同一页面提供独立的“公司级交易重置”高风险区，封装 ERPNext `Transaction Deletion Record`；必须先预检并展示 DocType/记录数量，再要求不可逆确认复选框和完整确认文本。公司级重置不能复用普通模板 reset 的确认流程或白名单。
- 前端权限和按钮禁用只用于交互保护，生产环境拒绝、公司白名单和 `System Manager` 权限始终以后端校验为准。
...
- `/master-data/products`：商品列表、创建、进入维护工作区、启停、批量启停、批量修改分类 / 品牌、CSV 批量导入、当前筛选结果导出
- `/master-data/products/:itemCode`：商品只读详情、资料质量、库存、单位、完整价格矩阵、条码列表、最近库存流水
- `/master-data/products/:itemCode/edit`：现有商品全屏维护工作区，集中承载基本资料、单位与包装、销售/采购价格、条码、库存估值和变更历史
...
- 批量启停和批量修改保存每个选中商品的 `itemCode + modified` 快照，逐条调用受乐观锁保护的正式接口。批量语义为“逐条独立提交、允许部分成功”，页面必须展示每个商品的成功/失败结果，不能用一个笼统成功提示掩盖并发冲突，也不能因单条失败丢失已成功记录的反馈。
- 列表导入能力支持下载 CSV 模板、上传后预览校验，并按行执行 `create` / `update`；当前为前端顺序调用既有商品创建 / 更新接口，适合小批量维护
- 新增商品继续使用创建弹窗；现有商品不再进入该弹窗，列表、详情和 AI 商品快捷窗口统一跳转全屏维护工作区
- 工作区普通资料保存支持图片、名称、分类、品牌、主条码兼容字段、描述、启停、包装换算、批发 / 零售默认单位和库存估值；库存基准单位锁定，错误纠正进入受控迁移流程
- 商品详情页支持多条码管理，包括新增条码、删除条码和设置主条码；条码列表使用 ProTable 展示主条码状态和顺序，基础编辑表单中的 `barcode` 仍只作为主条码兼容字段
- 商品详情价目表通过 `listProductPrices` 读取当前账号可见的全部正式价格，不再只显示 Standard Selling / Wholesale / Retail / Standard Buying 四个摘要；销售和采购价格矩阵按价格表、币种、单位和有效期逐条展示。
...
- 商品详情页遵循同一 Item 权限与版本规则：编辑、启停、单位纠正和条码动作不得形成绕过工作区的旧入口；详情页发生版本冲突时同样保留可恢复提示。资料质量区的编辑入口也必须随 Item 写权限禁用。
...
- 当前商品导出为前端 CSV 导出，按当前筛选条件以 `name asc` 稳定排序、每页 100 条读取 `listProducts`，完整性校验通过后一次生成文件；浏览器同步上限为 5000 条，超限不生成截断文件，后续大数据量导出再补后端异步导出接口
...
- `src/components/UomSelect.tsx`
  - 通过 `listUoms` / `list_uoms_v2` 查询启用且属于“业务可选”目录的单位，避免 ERPNext 内置科学单位进入日常表单。
...
- 直接列出关联发票和发货 / 收货单链接；销售侧提示先取消发票详情中的原客户收款，再作废销售发票，最后取消发货单；采购侧提示先取消最近付款，再取消发票，最后取消收货单。
...
Web 端已建立打印领域 service、通用单张打印组件、独立预览页和批量打印组件：

- 单张打印入口为 `src/components/PrintDocumentButton.tsx`；业务详情页只传 `doctype` 和 `docname`。
- 打印预览统一进入 `/printing/preview`，由独立页面调用 `get_print_preview_v1` 并在 iframe 中展示后端 HTML，避免异步请求结束后打开窗口被浏览器拦截。
- 独立预览页支持切换可见模板、刷新、系统打印和 PDF 下载；系统打印审计表示“已触发浏览器打印对话框”，不表示用户一定完成了纸张输出。
- PDF 下载先调用 `myapp.api.gateway.get_print_file_v1` 获取文件元数据，再通过 `myapp.api.gateway.download_print_file_v1` 下载真实文件流。
- 当前已接入 `Sales Order`、`Delivery Note`、`Sales Invoice`、`Purchase Order`、`Purchase Receipt`、`Purchase Invoice`、`Payment Entry` 七类单据详情页。
- 通用打印按钮会在下拉展开时调用 `get_print_templates_v1`，按后端返回的模板分类、说明和默认模板展示菜单；页面层只传 `doctype` 和 `docname`，不要硬编码模板列表。
- 前端在模板元数据尚未加载时不假定 `standard`；未选择模板时不传 `template`，由后端按全局设置和当前用户角色解析默认模板。
- 当前 7 类核心单据均已有第二模板变体：发票和收付款凭证 `finance`、订单 `external`、发货 / 收货 `warehouse`。变体模板已包含实际业务差异：财务版增加收付 / 未结 / 核销复核信息，对外订单版增加确认条款与签章栏，仓库版增加拣货 / 收货 / 复核执行栏。
- 七类交易单据列表均已接入批量选择和 `PrintBatchAction`：销售 / 采购订单、发货 / 收货、销售 / 采购发票和收付款流水均可创建异步 PDF 批次；Drawer 支持取消、失败项重试、成功项 ZIP 下载和合并 PDF，单批前后端均限制最多 100 张单据。
- Web 创建批次时生成 `request_id`，后端数据库唯一约束保证重复提交只返回原批次。
- 批次处于 `queued`、`processing`、`cancel_requested` 时自动轮询，进入 `completed`、`partial_failed`、`failed`、`canceled` 后停止。
- `/printing/settings` 为 `System Manager` 打印设置页，合并 `list_print_doctypes_v1` 和 `get_print_settings_v1` 展示 7 类单据，并通过 `set_print_default_template_v1` 维护全局默认模板。
- Web 已新增一级菜单 `/printing`：
  - `/printing/batches`：服务端分页查询批量打印任务，支持状态、日期和申请人筛选；普通用户只看到自己的任务，打开详情后继续轮询并支持取消、失败重试和 ZIP 下载。
  - `/printing/history`：服务端分页查询跨单据打印历史，支持单据类型、单据号、动作、状态、模板、日期和操作人筛选，并可跳回业务单据或进入统一预览页补打。
  - `/printing/settings`：仅 `System Manager` 可见。
  - `/printing/preview`：隐藏菜单的统一预览页。
- 批次列表只读取摘要，逐单 `results[]` 在用户打开 Drawer 后通过 `get_print_batch_v1` 加载，避免列表接口传输大量结果 JSON。
- `table_ready=false` 必须显示站点迁移提示，不得把治理表未创建伪装成空数据。
...
- `/sales/orders/:name` 已接入订单详情、金额汇总、履约信息、关联发货单 / 发票、业务时间线和商品明细，并支持按明细行填写本次数量创建发货单 / 销售发票、选择具体销售发票并按金额和付款方式登记客户收款、回退并修改订单、取消销售订单，以及动作入口不可用时的原因提示；订单详情提供“一键开单”入口，对已有订单按当前状态串行创建销售发货单和销售发票，若订单已经无需发货则只创建销售发票，库存不足时必须二次确认后才能强制发货并开票；订单已结清时应禁用登记客户收款入口；“回退并修改订单”只要检测到客户收款，回退弹框就应列出收款单、日期、方式、金额和“取消收款”操作。一笔客户收款允许用户先手动取消，也允许在额外二次确认后向 `quick_cancel_order_v2` 传 `rollback_payment=1` 同步取消；多笔客户收款必须逐笔取消，全部取消后再继续回退销售发票和销售发货单；Web 端当前隐藏直接发起销售退货 / 退款核对按钮，订单关联单据也不展示退货发票 / 退款单两项，业务改错应使用“回退并修改订单”或进入销售发票 / 销售发货单详情按顺序作废相关单据。
- `/sales/delivery-notes` 已接入销售发货单列表，支持关键词、公司、日期、单据状态、排序和分页。
- `/sales/delivery-notes/:name` 已接入销售发货单详情、金额 / 数量汇总、收货信息、关联销售订单 / 销售发票和商品明细，并支持作废发货单；详情页应沿用销售订单详情的 Ant Design Pro 风格：顶部 KPI，中间主区域展示商品明细和后续处理，右侧展示单据属性、关联单据、收货信息和回退动作；详情页需要展示“后续处理 / 历史单据说明”，已开票时引导查看销售发票，未开票且有关联订单时引导返回订单详情并通过 `?action=invoice` 进入开票动作。
- `/sales/invoices` 已接入销售发票列表，支持关键词、公司、日期、单据状态、排序和分页。
- `/sales/invoices/:name` 已接入销售发票详情、金额 / 收款汇总、收款历史、收款信息、关联销售订单 / 发货单和商品明细，并支持登记客户收款、作废销售发票和取消客户收款凭证；详情页应沿用销售订单详情的 Ant Design Pro 风格：顶部 KPI，中间主区域展示收款历史和商品明细，右侧展示单据属性、关联单据、结算信息和原收款处理动作；详情页需要展示“流程承接 / 历史单据说明”，有未收金额时直接登记客户收款，已结清时引导查看发货单或打印留档。收款历史来自 `get_sales_invoice_detail_v2.payment.entries[]`，应展示每笔客户收款的收款单号、日期、付款方式、核销金额、实收金额、差额核销、多收保留和参考号，收款单号应链接到 `/payments/<收付款单号>`，并提供固定在右侧的行级“取消这笔收款”入口。取消客户收款是作废某一个 `Payment Entry` 的纠错 / 回退动作，不是正式客户退款；多笔收款必须让用户看清并选择具体收款单，确认弹窗应展示金额、方式、日期和参考号，顶部入口只能表述为“选择取消客户收款”或“选择清理异常收款”，不能默认取消最近一笔；作废销售发票必须使用明确的二次确认框展示发票金额、关联客户收款、销售订单和发货单，一笔客户收款允许用户先手动取消，也允许在额外风险确认框中同步取消收款并作废发票，多笔客户收款必须逐笔取消，全部清理后才允许确认作废发票。
...
- `/sales/returns/new` 保留页面和服务封装，但 Web 端当前暂停直接发起销售退货，订单 / 发货单 / 发票详情不渲染退货和退款核对按钮，订单关联单据不展示退货发票 / 退款单两项，直接访问该页面只展示暂停说明。历史已经生成的退货发票仍保留在时间线和回退链路数据中，供后续恢复功能或异常回退使用。
...
- 使用 `RemoteLinkSelect` 选择客户、公司和仓库，客户变化后立即调用 `get_customer_sales_context` 自动带出默认公司、仓库、联系人、联系电话和地址；这些值只是本次订单初始值，用户仍可在订单表单里临时修改，不回写客户主数据。
- 新建和编辑订单的联系电话必须使用 `src/utils/phone-validation.ts` 归一化和校验，只允许输入数字，提交前再次标准化；客户上下文或历史订单带出的电话也要先过滤非数字再写入表单。
...
- 交易选品搜索区应提供“库存仓库”筛选，默认留空表示全部仓库；关闭并重新打开 Drawer 必须清空关键词、分类、品牌、库存范围和仓库筛选，避免上一次筛选缓存影响新一轮选品。选择具体仓库时可自动按该仓有库存过滤候选；筛选只影响库存查看、库存过滤和候选范围，不决定最终明细行入库 / 出库仓，最终仓库仍以行内默认值或“调整”抽屉为准。库存范围筛选使用下拉项“全部商品 / 仅有库存”，不要使用搜索表单内难点击的开关控件。
- 订单选品列表必须固定商品列和右侧操作列。交易选品点击“加入”应直接写入订单明细并保持 Drawer 打开，不再引入暂存、勾选和批量加入的第二套中间状态。默认行展示系统带入的单位、仓库、价格、销售模式和数量摘要，特殊修改通过“调整”抽屉完成，避免把所有字段都暴露成必须手动填写的表格输入。
...
- 交易选品 Drawer 右侧应常驻展示“本单已加入”，用于解释订单当前已有业务行和同 SKU 多行来源；每行业务信息必须展示数量 + 单位、金额、仓库、模式、单价，并使用共享 UOM / 换算 / 金额工具生成展示值。右侧合计区放在底部，展示商品行、数量和金额，不能挤占顶部明细列表的扫读空间。不要要求用户再打开二级抽屉才能确认已选内容。非交易类弹窗选品可继续使用“已选明细”抽屉展示暂存业务行。
...
- 编辑页底部保存区应同时展示未保存状态、商品行数、数量和订单金额，并提供取消与保存修改。`FooterToolbar` 内的汇总区必须可换行并给操作按钮保留固定空间，避免总金额和按钮在窄屏或浏览器缩放时重叠。表单、商品明细、添加商品和批量切换销售模式都应标记为未保存；浏览器刷新或关闭前应提示未保存修改。
- 收货地址、备注等从后端读取到文本框前要清理 HTML 标签和 `<br>`，转成真实换行，避免用户看到后端展示格式。
- 商品明细表金额列和合计金额应突出显示，数量输入遵循“整数不显示无意义小数、可直接输入并保留增减按钮”的统一规则。
- 编辑页继续复用增强版 `ProductSelect`，并传入当前明细业务行用于弹窗内回显当前维度已加和同 SKU 总已加。已存在下游单据或已结清订单会被编辑入口阻断，不应打开选品弹窗。
- 基础信息保存调用 `update_order_v2`；商品明细替换调用 `update_order_items_v2`。
- 已发货、已开票、已结清、已完成、已作废或非已提交订单不能直接编辑。订单详情页必须禁用“编辑订单”入口并展示原因；用户直接访问 `/sales/orders/:name/edit` 时也必须显示阻断页。需要改错时应先按客户收款、销售发票、销售发货单顺序回退下游单据，再编辑仍然有效且未进入下游流程的销售订单。
...
- 销售发货单详情页的回退说明优先展示 `actions.cancel_delivery_note_hint`；作废发货单前必须使用明确确认框提示库存扣减和订单履约状态会回退，并展示发货单、来源销售订单、关联销售发票、发货数量和发货金额。只要发货单已关联销售发票，页面必须禁用作废入口并提示先进入销售发票详情作废下游发票，再回到发货单作废；不要绕过后端 `cancel_delivery_note` 的下游发票拦截。
- 销售发票详情页的回退说明优先展示 `actions.cancel_sales_invoice_hint`；如果发票存在客户收款单，作废发票前应提示收款处理风险。一笔客户收款允许额外二次确认后同步取消收款并作废发票；多笔客户收款场景不能自动只取消最近一笔并继续作废发票，应要求用户在收款历史逐笔取消。
...
- 订单详情右侧关联单据和退款入口应复用 `timeline[]` 中的收款单、退货发票和退款单信息；Web 端当前暂停新的退货入口时，关联单据只展示发货单、销售发票和收款单，不展示退货发票 / 退款单两项；后续恢复退货退款入口后，没有退货发票时禁用退款核对入口，当存在多张退货发票时必须让用户选择具体单据，不能默认取第一张。
...
- `/payments/:name` 已接入 `get_payment_entry_detail_v1`，用于查看单笔收付款凭证、业务链路、核销明细、差额 / 扣减明细、关联销售 / 采购 / 退货发票和作废动作状态；当 `actions.can_cancel=true` 时，详情页允许直接作废这张 `Payment Entry`，主作废入口应放在顶部收款数据旁，右侧动作状态只保留说明和辅助确认入口；确认弹窗必须展示影响的核销明细，并明确这不是正式退款操作。
...
- 取消收款 / 付款的核心对象是 `Payment Entry` 本身。销售发票或采购发票详情页可以保留上下文入口，但资金详情页也必须允许财务人员从收付款单号进入后作废凭证；列表页不直接放危险操作，避免未查看核销范围就误操作。
...
- `/master-data/uoms` 已接入计量单位列表，支持关键词、启停状态、业务可选目录筛选、分页查询、新增、编辑、启用和停用；系统启用与日常业务可选是两个独立治理维度。
...
- 商品轻量维护当前覆盖基础字段、图片、库存基准单位、商品单位换算表、批发 / 零售默认单位和标准价格。编辑已有商品时库存基准单位锁定，普通保存完整保留单位换算表且只提交本次实际修改的价格字段；单位错误通过低风险原地纠正或高风险继任商品流程处理。库存目标数量调整和批量盘点均提交 ERPNext `Stock Reconciliation`，库存转仓继续提交 Material Transfer。
...
- `/welcome`、`/list` 等仍未清理的模板路由；原 `/admin` 已替换为真实 `/administration` 用户治理路由
- mock 数据对真实业务页面的影响
...
- `/welcome`、`/list` 暂时保留但已从菜单隐藏；原 `/admin` 已替换为真实 `/administration` 用户治理路由，根路由已调整到 `/dashboard`。
...
截至 2026-07-04 当前本地提交，销售收款 / 退货退款 / 回退链路已完成以下收尾：

- Web 端暂停直接发起销售退货入口：订单、发货单和发票详情隐藏新建退货和退款核对按钮，`/sales/returns/new` 保留页面但展示暂停说明；历史退货发票仍可查看、退款核对和回退。
- Web 销售订单“回退并修改订单”已收紧收款安全确认：`quickCancelSalesOrderV2` 默认传 `rollback_payment=0`；只要存在客户收款，回退弹框就展示收款列表和“取消收款”入口；一笔客户收款时，用户也可以在额外风险确认框中选择“取消收款并一键回退”后传 `rollback_payment=1`；多笔客户收款时，未全部逐笔取消前不允许继续回退发票和发货单。
- Web 销售订单详情已补“一键开单”：对已有订单先弹出确认框，再按当前状态创建销售发货单和销售发票；库存不足时必须二次确认后才允许强制发货并开票。
...
本文档不固定记录分支 ahead 数、最新提交号或工作区是否干净，这些状态会随开发快速变化。当前仓库状态、最近验证和下一步以父仓库 `docs/codex/CURRENT_HANDOFF.zh-CN.md` 为准；全项目未满足需求和优先级以 `docs/codex/PROJECT_GAP_ROADMAP.zh-CN.md` 为准。
...
- 应用标题、PWA 名称、菜单、登录页标题和页脚已改为中文业务后台文案。
- `/welcome`、`/list` 模板路由暂时保留但已从菜单隐藏；原 `/admin` 已由真实的 `/administration` 用户治理路由替换。
...
- `UomSelect` 固定请求 `enabled = 1` 且 `business_selectable = 1`；已有记录的历史单位通过当前值和 `displayValue` 回显，不因退出业务目录而被普通保存清空。
...
- 商品价格提交必须携带计价 `uom`：标准售价和标准采购价使用库存单位，批发价使用批发默认单位，零售价使用零售默认单位；价格明细表同时展示价格表、单位、币种和金额。
- 商品新增条码必须从当前商品 `all_uoms` 选择对应单位，条码表同时展示单位；不得把件码、箱码作为无单位字符串混在一起。
...
- 个人设置头像使用真实文件上传，不再要求用户手填图片 URL；上传前统一进行 1:1 裁剪、缩放、旋转并输出 512 × 512 WebP，已有头像支持重新裁剪。
...
- AI 库存调整将“本次计价单位价格”和“执行后库存估值单价”分开显示：前者按用户选择的件、箱、包等单位录入或采用采购价，后者是折算后写入 ERPNext Stock Reconciliation 的库存基准单位 `valuation_rate`。页面优先沿用当前仓库 Bin 实际估值，并展示当前/执行后库存数量、当前/执行后库存价值、价值差额；估值变化且已有库存时必须突出提示会重新估值全部已有库存。
- 库存草稿展示 Backend 返回的全部有效采购价格候选，不限于 Standard Buying。候选表必须显示本地化价格表名称、原价格与币种、计价单位、换算系数、折算后库存单位估值、有效期和采用状态。箱价与件价是独立正常价格，不把折算结果不同误报为冲突；当前估值为零时，所选单位只有一个候选可自动采用，多个同单位候选或只有其他单位候选必须由用户明确选择或人工填写。采用候选时保存 `reference_id`，Backend 会重新核验 Item Price，页面不得把客户端折算值当作权威来源。采购价本身只作候选，只有采用并转换为正式 `valuation_rate` 后才影响库存价值，也不会创建采购单、采购发票或供应商应付。
- 库存调整原因提供盘盈、盘亏、期初库存校准、历史数据纠正、单位或包装换算纠正、破损/报废/过期损耗等快捷项，并保留“其他”手工填写；快捷项只减少输入，不替代正式原因字段、版本校验或执行前确认。
...
- `/ai/drafts` 是当前用户草稿中心，支持销售订单、采购订单、库存调整和商品建档草稿的筛选、详情、校验结果、来源会话、放弃和安全交接。商品建档交接到 `/master-data/products`，复用既有 `create_product_v2`；草稿进入业务编辑器后仍需用户主动保存，AI 不创建正式商品、价格、库存或交易单据。
- AI 工作台和草稿中心的四类草稿编辑必须共用统一编辑器，并在每次打开时按草稿 ID 重新读取最新持久版本；不得直接使用会话 citation 或 ProTable 行快照初始化表单。保存与历史恢复必须传递用户当前看到的 `expectedVersion`，并通过 `runGatewayMutation` 携带幂等键；版本冲突不得静默覆盖。保存后必须采用后端返回的新 payload、validation 和 version 刷新界面，采购草稿还需保留币种与供应商参考号。
- 草稿中心详情使用业务字段、商品明细和库存变化进行复核，并提供不可变版本差异与安全恢复；原始 JSON 仅保留为诊断页签。
- 商品、订单、经营报表和四类草稿能力均已接入；AI 安全边界仍保持不变：不能创建、提交、取消、收付款或直接修改库存。
...
- 模型管理顶部概览聚合 Orchestrator 可达性、近 7 日请求/错误/成本、模型/策略数量和向量状态；模型同步展示当前 LiteLLM Key 可见库存，一键可用性检查会发起最小真实请求并可能产生少量 Provider 费用；用量页面提供近 30 日请求、成功率、成本、p95 与趋势图。
