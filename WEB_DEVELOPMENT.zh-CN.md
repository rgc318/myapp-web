...
- `/sales/returns/new` 保留页面和服务封装，但 Web 端当前暂停直接发起销售退货，订单 / 发货单 / 发票详情不渲染退货和退款核对按钮，直接访问该页面只展示暂停说明。历史已经生成的退货发票仍可通过订单时间线、关联单据和退款核对页面继续查看或回退。
