...
- `src/components/RemoteLinkSelect.tsx`
  ...
  - 页面只传 `doctype`、`extraFields`、`placeholder` 和 `onChange`。
...
- `/sales/orders/:name` 已接入订单详情、金额汇总、履约信息、关联发货单 / 发票和商品明细，并支持按明细行填写本次数量创建发货单 / 销售发票、按金额和付款方式登记收款、快捷回退下游单据、取消销售订单。
...
- 当前订单存在多张发货单、多张销售发票或多笔有效收款时，后端会拒绝快捷回退，页面提示用户改用分步回退。
...
- `/purchase/orders/:name` 已接入采购订单详情、金额汇总、基本信息、供应商信息、关联收货单 / 发票和商品明细，并支持按明细行填写本次数量创建收货单 / 采购发票、在单张关联采购发票场景按金额和付款方式登记付款、快捷回退下游单据、取消采购订单。
...
- 当前订单存在多张采购收货单、多张采购发票或多笔有效付款时，后端会拒绝快捷回退，页面提示用户改用分步回退。
