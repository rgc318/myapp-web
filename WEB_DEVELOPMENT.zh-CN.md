...
- `/master-data/warehouses` 已接入仓库列表，支持关键词、公司、状态、仓库类型筛选，支持新增、编辑、启用和停用；当前覆盖仓库名称、公司、父仓库、是否分组、会计科目、仓库类型、默认在途仓库、拒收仓标记、客户归属、联系方式和地址等 ERPNext 原生治理字段。
...
- 仓库管理当前已接入 ERPNext 原生基础治理字段；库位 / 容量、负责人、默认成本中心、仓库权限、导入导出、审计记录和更细粒度治理仍归后续模块。
