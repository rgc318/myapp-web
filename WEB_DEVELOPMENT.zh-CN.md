...
- `/sales/refunds/review` 已接入销售退款核对，支持读取来源销售发票收款状态、收款历史、查看退货发票，并在需要回退原收款时取消最近收款；当前不伪造独立打款退款接口。
...
- 当前后端没有独立客户退款打款接口；页面只提供现有可执行的“取消最近收款”回退动作。
- 如业务已经线下退款，应保留财务凭证并按实际流程登记，不在 Web 中伪造退款完成状态。
