...
- AI 库存调整增加库存时展示“库存单位成本”，优先沿用有效库存估值；没有有效估值但存在 Standard Buying 时自动带出为标准采购参考价建议。同单位直接采用，其他已配置商品单位按 conversion factor 折算为每库存基准单位成本，并展示原价格、单位和换算系数；缺少换算关系时不自动采用。同一商品多条 Standard Buying 折算后不一致时保持成本字段为空，并展示每条原价与库存单位折算结果，要求用户确认价格单位或人工输入。界面同时显示来源与“不会生成采购单/应付”的提示。用户修改后来源记为 `user`。标准采购参考价不能在界面中简称为“成本价”，以免与库存实时估值和采购成交价混淆。
